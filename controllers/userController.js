...
async function getAllDoctors(req,res){
  try{
    const {name,specialty,hospital_name,rating}=req.query;
    let filter={};
    ...
    const doctorsWithAvailabilityAndRatings=await Doctor.aggregate([
      {
        $match:filter,
      },
      {
      ...
      {
        $match:{$expr:{$gte:["$averageRatings",parseFloat(rating)]}},
      },
      {
        $project:{
          _id:0,
    ...
    res.json(doctorsWithAvailabilityAndRatings);
  }catch(err){
    console.error(err.message);
